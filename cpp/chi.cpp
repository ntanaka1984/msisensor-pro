...
double get_chisqr_p(unsigned short * first, unsigned short * second) {
	...
	std::cerr << CriticalValue << "\t" << NumberOfEffectiveDatapoints << "\t"
			<< returnP << std::endl;
...
double X2BetweenTwo(unsigned short * FirstOriginal,
	...
	if (paramd.Normalization == 1) {
		...
		if ((sum_first > sum_second) && (sum_second != 0)) {
			for (int i = 0; i < dispots; i++) {
				SecondOriginal[i] = SecondOriginal[i]
	...
	for (unsigned i = 0; i < dispots; i++) {
		if (FirstOriginal[i] + SecondOriginal[i] > 0.0)
			Degree++;
		if (ExpFirst[i])
			result += (FirstOriginal[i] - ExpFirst[i])
					* (FirstOriginal[i] - ExpFirst[i]) / ExpFirst[i];
		if (ExpSecond[i])
			result += (SecondOriginal[i] - ExpSecond[i])
					* (SecondOriginal[i] - ExpSecond[i]) / ExpSecond[i];
	}
